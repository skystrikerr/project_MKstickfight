...
npm run get:exe v0.6.2 # a specific tagged release instead
```
...
npm run dist:win       # -> release/Plank Fighter World-0.6.2-x64.exe   (needs Windows)
npm run dist:linux     # -> release/Plank Fighter World-0.6.2-x64.AppImage
npm run dist:mac       # -> release/Plank Fighter World-0.6.2-x64.dmg   (needs macOS)
```

...
import earthMap from "@/assets/ui/earth-map.webp";
...
export function FighterWorld3D({ initialFighterId = "roman", onSelectFighter, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    // Imported rather than resolved with `new URL(..., import.meta.url)`.
    // That form leaves the file sitting in assets/, and this game also ships
    // as one packed HTML file and as a desktop build opened straight off
    // disk - in both of those the globe would have come up bare.
    const textureUrl = earthMap;
    ...
    function resize() {
      // `mount` is captured from the ref and already guarded above, but a
      // function declaration is hoisted out of that narrowing, so it is
      // re-read from the element the renderer is actually attached to.
